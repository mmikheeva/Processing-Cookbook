...
1. Update Cookbook-instructions.txt script: (1) specify input and output directories; (2) specify tools at each processing step, or select `Skip` if the step is skipped; (3) add/remove required arguments for each tool. Save the updated instruction file.
